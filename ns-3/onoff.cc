...
/*
static void
CalculateDelay(Ptr<PacketSink>pktSink)
{
    cout << "Rx = " << PacketSink-> GetTotalRx() << "bytes(" << pktSink->GetTotalRx()*8 / (stopTime - startTime) << "bps)," << endl;
}
*/
 ...
 main (int argc, char *argv[])
 {
   ...
   LogComponentEnable ("onoff", LOG_LEVEL_INFO);
   ...
   // tables in the nodes.
    ...
    for(double t=0.1;t<0.3;t+=0.1)
    {
        ...
        for(int i=0;i<hostNumber;++i)
        {
            ...
            clientHelper.SetAttribute ("MaxPackets", UintegerValue (27));
            clientHelper.SetAttribute ("Interval", TimeValue (Seconds (0.00375)));
            clientHelper.SetAttribute ("PacketSize", UintegerValue (210)); 
            
            ApplicationContainer apps;  
            
            //n0->n7  
            AddressValue remoteAddress(InetSocketAddress (interfaces[hostPairs[i]+49].GetAddress (1), port));  //目的地址
            clientHelper.SetAttribute("Remote",remoteAddress);  
            apps = clientHelper.Install(r.Get (hostPairs[i+hostNumber]+30));  //发送节点
            apps.Start(Seconds(t));  
            apps.Stop(Seconds (t+0.1)); //60s
            //apps.Get(0)->TraceConnectWithoutContext("Rx", MakeCallback(&CalculateDelay));
            ++port;
        }
        
    }
    
    p2p.EnablePcapAll("onoff"); 

    //Ptr<PacketSink>pktSink;
    //std::cout << "Rx = " << PacketSink-> GetTotalRx() << "bytes(" << pktSink->GetTotalRx()*8 / (stopTime - startTime) << "bps)," << endl;
   ...
   Simulator::Run ();
   NS_LOG_INFO ("Done.");
   flowmonHelper.SerializeToXmlFile ("onoff", false, false);
